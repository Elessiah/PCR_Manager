...
### Dev auth bypass (browser-only mode)

`npm run dev` démarre le frontend Vite sur `http://localhost:1420` **sans backend Rust**.

**Stratégie** : `vite.config.ts` aliase `@tauri-apps/api/core` → `src/lib/tauri-proxy.ts` quand `mode !== 'production'`. Le proxy :
- Si `window.__TAURI_INTERNALS__` existe (contexte `tauri:dev`) → délègue à l'invoke natif.
- Sinon (browser pur) → appelle `devMockInvoke()` dans `src/lib/__mocks__/dev-data.ts`.

**Comportement du mock** :
- `session_check` retourne `{ authenticated: true }` → l'utilisateur est directement authentifié, aucun code TOTP requis.
- `mac_auth_available` et `totp_available` retournent `false` → la LoginPage passe en mode "setup", mais `AuthContext` bypasse via `session_check`.
- Toutes les commandes CRUD (travailleur, appareil, habilitation, compétence…) sont gérées avec un **store in-memory stateful** : les créations/modifications/suppressions persistent pendant la session browser.
- Au rechargement de la page, le store repart de zéro (état vide sauf l'établissement par défaut).

**Pour les tests E2E Puppeteer** : lancer `npm run dev`, puis piloter via Puppeteer sur `http://localhost:1420`. L'authentification est automatiquement bypassée.
